...
export default function AdminApprovedAdsPage() {
  ...
  const fetchAds = async () => {
    setLoading(true);
    try {
      const response = await adminPanelApi.getApprovedAds({
      ...
      toast.error("خطا در دریافت آگهی‌های تایید شده");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold">آگهی‌های تایید شده</h1>
        <p className="text-sm text-muted-foreground">
          لیست آگهی‌هایی که توسط کارشناسان تایید شده‌اند
        </p>
      </div>
