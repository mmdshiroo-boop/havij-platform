...
export default function MarketAnalysisPage() {
  ...
  const [regions, setRegions] = useState<string[]>(["همه"]);
  ...
  // ---------- دریافت داده‌های اصلی بازار (تحلیل + نقشه) ----------
  const fetchMarketDashboardData = useCallback(async () => {
    ...
    try {
      ...
      if (analysisRes.data?.success) {
        ...
        // ذخیره لیست مناطق/محله‌ها با تعداد آگهی واقعی
        setTopDistricts(
  ...
  // ---------- دریافت استان‌ها از نشان (برای فیلتر) ----------
  useEffect(() => {
    (async () => {
      try {
    ...
    (async () => {
      try {
        const res = await apiClient.get(
          `/market/neshan-cities-list?province=${prov.name}`,
        );
        if (res.data?.success)
          setCities(
  ...
  // ---------- دریافت محله‌ها برای فیلتر (فقط در صورت انتخاب شهر) ----------
  useEffect(() => {
    if (!selectedCityId) {
    ...
    (async () => {
      try {
        const res = await apiClient.get(
          `/market/neshan-districts-list?city=${city.name}`,
        );
        if (res.data?.success)
  ...
  // ---------- کلیک روی محله (در حالت شهر انتخاب‌شده) ----------
  const handleDistrictClick = useCallback(
    async (districtName: string) => {
      ...
      try {
        ...
        const res = await apiClient.get("/market/map-ads", {
          params: {
  ...
  const handleShowAdsOnMap = async (districtName: string) => {
    ...
    try {
      ...
      if (data?.[0]) {
        ...
        if (!isNaN(lat) && !isNaN(lon)) {
          setDynamicCenter([lat, lon]);
          setMapZoom(city ? 16 : 10);
          toast.success(`نقشه روی ${districtName} متمرکز شد`);
        } else toast.error("مختصات معتبر نیست");
      } else toast.error("مختصات یافت نشد");
    } catch {
      toast.error("خطا در دریافت مختصات");
    }
  };
      ...
      <div className="min-h-screen bg-gradient-to-b from-background via-background to-muted/20">
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 py-6 space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 h-[450px] rounded-2xl overflow-hidden border border-border/50 shadow-xl">
              <MapView
                markers={markers}
                loading={loading}
...
// ============ SUB-COMPONENTS (دقیقاً مطابق فایل اصلی شما) ============
// ========== SUB-COMPONENTS (همگی کامل) ==========
...
function TopDistrictsCard({
  ...
  const filtered = useMemo(
    () =>
      districtSearch.trim()
        ? sorted.filter((d: any) => (d.district || "").includes(districtSearch))
        : sorted,
        ...
        {loading ? (
          ...
          <div className="space-y-1">
            {filtered.map((item: any, idx: number) => {
              ...
              const isSelected = isCitySelected && analysisDistrict === name;
      ...
      <CardHeader className="p-4 pb-2 border-b border-border/20 bg-gradient-to-r from-background to-muted/5">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-4">
            <CardTitle className="text-sm font-black flex items-center gap-2">
              <BarChart3 className="w-4 h-4 text-primary" /> نوسان قیمت هر متر
              مربع
            </CardTitle>
