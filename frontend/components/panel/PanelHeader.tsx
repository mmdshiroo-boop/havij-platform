...
> = {
  user: [
    ...
    { href: "/panel/user/tickets", label: "تیکت‌های من", icon: MessageSquare },
    { href: "/panel/user/profile", label: "پروفایل کاربری", icon: User },
    { href: "/panel/user/settings", label: "تنظیمات سیستم", icon: Settings },
  ],
  vip: [
    { href: "/panel/vip", label: "داشبورد ویژه", icon: LayoutDashboard },
    { href: "/panel/vip/my-ads", label: "آگهی‌های ویژه VIP", icon: Crown },
    ...
    { href: "/panel/vip/benefits", label: "مزایا و تخفیف‌ها", icon: Gift },
    { href: "/panel/vip/upgrade", label: "ارتقا اشتراک", icon: Star },
  ...
  agent: [
    ...
    { href: "/panel/agent/bookmarks", label: "ذخیره‌شده‌ها", icon: Bookmark },
    {
      ...
      label: "بانک اطلاعات املاک",
      icon: Building,
    },
    { href: "/panel/agent/agents", label: "مدیریت مشاوران", icon: Users },
    { href: "/panel/agent/reports", label: "گزارشات فصلی", icon: FileText },
    { href: "/panel/agent/profile", label: "پروفایل حقوقی", icon: User },
  ],
  developer: [
    {
      href: "/panel/developer/dashboard",
      label: "کنسول توسعه‌دهندگان",
      icon: LayoutDashboard,
    },
  ...
  expert: [
    ...
    { href: "/panel/expert/profile", label: "پروفایل", icon: User },
  ],
  admin: [
    { href: "/panel/admin", label: "پیشخوان ادمین", icon: LayoutDashboard },
    { href: "/panel/admin/users", label: "نظارت بر کاربران", icon: Users },
    { href: "/panel/admin/ads", label: "کنترل جامع آگهی‌ها", icon: FileText },
    {
      href: "/panel/admin/tickets",
      label: "مدیریت تیکت‌ها",
      icon: MessageSquare,
    },
    { href: "/panel/admin/reports", label: "گزارشات", icon: FileText },
    {
      href: "/panel/admin/categories",
      label: "پیکربندی دسته‌بندی‌ها",
      icon: Settings,
    },
    { href: "/panel/admin/notifications", label: "اعلان‌ها", icon: Bell },
  ...
  super_admin: [
    ...
    {
      href: "/panel/super-admin/tickets",
      label: "مدیریت تیکت‌ها",
      icon: MessageSquare,
    },
    { href: "/panel/super-admin/reports", label: "گزارشات", icon: FileText },
    { href: "/panel/super-admin/backup", label: "پشتیبان‌گیری", icon: Clock },
    { href: "/panel/super-admin/logs", label: "لاگ‌های سیستم", icon: FileText },
    { href: "/panel/super-admin/notifications", label: "اعلان‌ها", icon: Bell },
    {
  ...
  const getRoleInfo = (userRole?: string) => {
    switch (userRole) {
      ...
      case "super_admin":
        return {
          title:
            userRole === "super_admin" ? "کنسول مدیر ارشد" : "کنسول مدیریت",
          badgeClass:
  ...
  return (
    ...
    >
      <div className="flex h-16 items-center justify-between px-4 md:px-6 max-w-[1500px] mx-auto">
        {/* === سمت راست: منوی موبایل + لوگوی اصلی بزرگ‌تر + نشان نقش === */}
        <div className="flex items-center gap-3">
          ...
          <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
            ...
            {/* کلاس [&>button]:hidden برای حذف باگ ضربدر تکراری اضافه شد */}
            <SheetContent
              side="right"
            ...
            >
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span
                      className={cn(
                        "inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border mt-0.5 w-fit",
                        roleInfo.badgeClass,
                      )}
                    >
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                <p className="text-[10px] font-black text-muted-foreground/60 px-3 py-1.5 uppercase tracking-wider">
                  دسترسی‌های سریع
                </p>
                  ...
                  {menuItems.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <ChevronLeft
                          className={cn(
                            "w-3.5 h-3.5 transition-all opacity-0 group-hover:opacity-100 group-hover:-translate-x-0.5",
                            isActive
              ...
              <div className="p-3 border-t border-border/40 bg-muted/20">
                <Link
                  href="/"
                  onClick={() => setMobileMenuOpen(false)}
                  className="flex items-center justify-center gap-2 w-full py-2.5 px-3 rounded-xl text-xs font-bold text-muted-foreground hover:bg-muted hover:text-foreground border border-border/40 transition-all active:scale-[0.98]"
                >
          ...
          {/* لوگوی بزرگ‌تر شده (log.png) در هدر دسکتاپ */}
          <Link
            href={getDashboardLink()}
            className="flex items-center gap-3 transition-transform hover:opacity-95 active:scale-95"
          >
            <img
              src="/log.png"
              alt="لوگوی اصلی"
              className="h-11 sm:h-12 w-auto object-contain shrink-0 py-0.5"
            />

            {/* نشان نقش کاربر */}
            <div
              className={cn(
                "hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-extrabold border backdrop-blur-md shadow-xs transition-all",
                roleInfo.badgeClass,
              )}
            >
        ...
        {/* === سمت چپ: دکمه بازگشت + اعلان‌ها + منوی کاربر === */}
        <div className="flex items-center gap-2 sm:gap-3">
          <Button
            variant="ghost"
            size="sm"
            className="gap-1.5 rounded-xl text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-all text-xs font-bold h-9 hidden md:flex"
            onClick={() => router.push("/")}
          >
