...
import SmsService, { getSmsService, SendSMSResult } from '../services/sms.service';
import { SmsLog, SmsTemplate } from '../models/sms.model';
/**
 * ============================================
...
class SmsController {
   ...
   */
  private async sendSMS(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
   ...
   */
  private async sendTestSMS(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { to } = req.body;
   ...
   * دریافت موجودی پیامک
   */
  private async getCreditBalance(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
   ...
   */
  private async getSmsLogs(req: Request<{}, {}, {}, PaginationQuery>, res: Response, next: NextFunction): Promise<void> {
    try {
   ...
   */
  private async getSmsStats(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
   ...
   */
  private async broadcastSMS(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      this.sendBulkInBackground(recipients, text, broadcastLog._id.toString())
        .catch((err) => {
   ...
   */
  private async getTemplates(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
   ...
   */
  private async seedTemplates(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
  ...
  private async sendBulkInBackground(
    ...
    await SmsLog.findByIdAndUpdate(broadcastLogId, {
      ...
      cost:
        Math.ceil(text.length / 70) * result.totalSent,
    }).exec();
  ...
  private async getAgentPhoneNumbers(): Promise<string[]> {
    try {
      ...
      const agents = await User.find({ role: 'agent', phone: { $exists: true, $ne: '' } })
        .select('phone -_id')
        .lean()
        .exec();
