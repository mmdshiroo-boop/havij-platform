...
export const editMessage = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user._id;
    const messageId = String(req.params.messageId); // ✅ تبدیل به رشته
...
export const uploadFileMessage = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user._id;
    const conversationId = String(req.params.conversationId); // ✅ تبدیل به رشته
    ...
    const uploadPath = path.join("uploads", "chat", fileName);
