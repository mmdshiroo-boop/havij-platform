...
export const getMarketAnalysis = async (req: Request, res: Response) => {
  try {
    ...
    // ✅ تبدیل امن تمام پارامترهای query
    const city = getQueryStr(req.query.city);
    const district = getQueryStr(req.query.district);
    const province = getQueryStr(req.query.province);
    const tradeType = getQueryStr(req.query.tradeType);
    ...
    const priceAnalysis = await Ad.aggregate([
      ...
      },
      {
        $project: {
          _id: 0,
          district: "$_id", // نام فیلد خروجی همچنان district است (برای یکسان‌سازی)
          totalAds: 1,
          avgPricePerMeter: {
            $cond: [
              { $gt: ["$areaCount", 0] },
    ...
    const topDistricts = priceAnalysis
      .filter((d) => d.district !== fallbackName)
      .slice(0, 30);
...
export const getMarketStats = async (req: Request, res: Response) => {
  try {
    let provinces = await Province.find({ isActive: true });
    if (provinces.length === 0) {
      ...
      await Province.insertMany(defaultProvinces);
      provinces = await Province.find({ isActive: true });
    }
    ...
    const stats = await MarketAnalysis.find().lean();
    const result: Record<string, any> = {};
    stats.forEach((stat) => {
      result[stat.provinceId] = {
        totalDeals: stat.totalDeals,
        avgPrice: stat.avgPrice,
        growth: stat.growth,
        soldDeals: (stat as any).soldDeals || 0,
        hotZones: stat.hotZones,
        propertyTypes: stat.propertyTypes,
      };
    });
    ...
    res
      .status(500)
      .json({ success: false, message: "خطا در دریافت آمار بازار" });
  }
};

// ======================== دریافت آمار یک استان خاص ========================
export const getRegionStats = async (req: Request, res: Response) => {
  try {
    const regionId = req.params.regionId as string;
    let province = await Province.findOne({ slug: regionId });
    if (!province) {
      // اگر استان در دیتابیس نبود، موقتاً یکی از defaultProvinces را استفاده کن
      const defaultProv = ({} as any)[regionId]; // ساده‌سازی
      if (!defaultProv)
        return res
          .status(404)
          .json({ success: false, message: "استان یافت نشد" });
      province = await Province.create(defaultProv);
    }

    let stats = await MarketAnalysis.findOne({ provinceId: regionId });
    if (!stats) {
      const updated = await updateProvinceStats(regionId);
      stats = updated;
    }
    if (!stats)
      return res
        .status(404)
        .json({ success: false, message: "آمار استان یافت نشد" });

    const allStats = await MarketAnalysis.find().sort({ totalDeals: -1 });
    const rank = allStats.findIndex((s) => s.provinceId === regionId) + 1;
    ...
    res.json({
      ...
      data: {
        ...
        growth: stats.growth,
        soldDeals: (stats as any).soldDeals || 0,
        hotZones: stats.hotZones,
        propertyTypes: stats.propertyTypes,
        rank,
        regionName: stats.provinceName,
        regionCode: stats.provinceCode,
      },
    });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ success: false, message: "خطا در دریافت آمار استان" });
  }
};
...
export const getHotZones = async (req: Request, res: Response) => {
  try {
    const regionId = req.params.regionId as string;
    const zones = hotZonesMap[regionId] || [
      "منطقه مرکزی",
      "منطقه شمال",
      "منطقه جنوب",
    ];
    ...
    console.error("Get hot zones error:", error);
    res
      .status(500)
      .json({ success: false, message: "خطا در دریافت مناطق داغ" });
  }
};
