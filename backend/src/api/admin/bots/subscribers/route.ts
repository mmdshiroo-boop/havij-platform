  ...
  try {
    switch (command) {
      case '/start': {
        ...
        await botService.sendMessage(chatId, welcomeText, platform, { buttons });
        break;
      }
      ...
      case '/categories': {
        ...
        await botService.sendMessage(chatId, categoriesText, platform, { buttons });
        break;
      }
...
async function performSearch(
  ...
  try {
    ...
    const platformButtons = platform === 'telegram'
      ? formatted.buttons
...
async function showAdDetails(
  ...
  try {
    ...
    if (!ad) {
      await botService.sendMessage(chatId, '❌ آگهی مورد نظر یافت نشد.', platform);
      return;
    }
 ...
 */
function formatWelcomeButtons(platform: BotPlatform): BotButton[][] | undefined {
 ...
 */
function formatCategoryButtons(platform: BotPlatform): BotButton[][] | undefined {
...
export async function broadcastMessageToAll(
  ...
  for (const sub of subscribers) {
    const success = await botService.sendMessage(sub.chatId, message, sub.platform);
    if (success) {
      sentCount++;
    } else {
      failedCount++;
